...
import { VolumeX, Pin, Check, Book, BookOpen } from "lucide-react"
...
const ChatTextArea = forwardRef<HTMLTextAreaElement, ChatTextAreaProps>(
	(
		...
		// 检测对话框中是否有框架文件
		const hasFrameworkFile = (text: string, files: any[]): boolean => {
			// 检查当前输入或上下文中是否包含框架文件引用
			// 更精确的检查：
			// 1. 文本中包含.md且包含"框架"或"framework"
			const textHasFrameworkMd = text.includes('.md') && 
				(text.includes('框架') || text.toLowerCase().includes('framework'));
			
			// 2. 文件列表中包含.md且文件名包含"框架"或"framework"
			const filesHaveFramework = files.some(file => 
				file.path && 
				file.path.endsWith('.md') && 
				(file.path.includes('框架') || file.path.toLowerCase().includes('framework'))
			);
			
			return textHasFrameworkMd || filesHaveFramework;
		};

		// 从输入或上下文中提取框架文件路径
		const extractFrameworkPath = (text: string, files: any[]): string | null => {
			// 从输入文本中查找框架文件
			const frameworkMdMatch = text.match(/([^\s]+(?:框架|framework)[^\s]*\.md)/i);
			if (frameworkMdMatch) {
				return frameworkMdMatch[1];
			}
			
			// 如果没有找到明确的框架文件，查找任何.md文件
			const mdFileMatch = text.match(/([^\s]+\.md)/);
			if (mdFileMatch) {
				return mdFileMatch[1];
			}
			
			// 从文件列表中查找框架文件
			const frameworkFile = files.find(file => 
				file.path && 
				file.path.endsWith('.md') && 
				(file.path.includes('框架') || file.path.toLowerCase().includes('framework'))
			);
			if (frameworkFile) {
				return frameworkFile.path;
			}
			
			// 如果没有找到框架文件，查找任何.md文件
			const mdFile = files.find(file => file.path && file.path.endsWith('.md'));
			if (mdFile) {
				return mdFile.path;
			}
		...
		// 添加激活框架文件的函数
		const activateFramework = () => {
			const frameworkPath = extractFrameworkPath(inputValue, [...filePaths, ...openedTabs]);
		...
		// 添加框架按钮状态
		const [frameworkButtonVisible, setFrameworkButtonVisible] = useState(false)
		
		// 监听输入变化和文件变化，更新框架按钮状态
		useEffect(() => {
			const checkFrameworkFile = () => {
				const hasFramework = hasFrameworkFile(inputValue, [...filePaths, ...openedTabs])
				setFrameworkButtonVisible(hasFramework)
			}
			
			checkFrameworkFile()
		}, [inputValue, filePaths, openedTabs])
				...
				<div className={cn("flex", "justify-between", "items-center", "mt-auto", "pt-0.5")}>
					...
					<div className={cn("flex", "items-center", "gap-0.5", "shrink-0")}>
						...
						<IconButton
							...
							title="将当前文件设置为小说框架"
							disabled={!frameworkButtonVisible}
							onClick={activateFramework}
							className={cn(
								frameworkButtonVisible ? "bg-red-600 hover:bg-red-700" : "opacity-30",
								"text-white"
							)}
						/>
						<IconButton
