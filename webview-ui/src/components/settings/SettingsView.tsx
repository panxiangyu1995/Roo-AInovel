...
import NovelFrameworkSettings from "./NovelFrameworkSettings"
...
const SettingsView = forwardRef<SettingsViewRef, SettingsViewProps>(({ onDone, targetSection }, ref) => {
	...
	const handleSubmit = () => {
		if (isSettingValid) {
			...
			vscode.postMessage({ type: "autoUpdateEnabled", bool: autoUpdateEnabled })
	...
	const sections: { id: SectionName; icon: LucideIcon }[] = useMemo(
		() => [
			...
			{ id: "novelFramework", icon: Book },
			{ id: "terminal", icon: SquareTerminal },
			{ id: "prompts", icon: MessageSquare },
			{ id: "experimental", icon: FlaskConical },
			{ id: "language", icon: AlertTriangle },
			{ id: "about", icon: Info },
		],
		...
		<Tab>
			...
			<div ref={containerRef} className={cn(settingsTabsContainer, isCompactMode && "narrow")}>
				...
				<TabContent className="p-0 flex-1 overflow-auto">
					...
					{/* 小说框架设置 */}
					{activeTab === "novelFramework" && (
						<NovelFrameworkSettings
							autoUpdateEnabled={cachedState.autoUpdateEnabled}
							setCachedStateField={setCachedStateField}
						/>
					)}
