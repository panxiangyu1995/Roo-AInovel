...
2、如果在planner模式中，请继续向用户提问，不要过早的使用此工具结束任务。在planner模式下，您应该持续使用ask_followup_question和novel_framework_refine等工具与用户互动，引导用户完善小说框架的各个部分，直到用户明确表示满意为止。
3、只有当用户明确表示已完成框架创建或不需要进一步修改时，才使用此工具结束planner模式的任务。
