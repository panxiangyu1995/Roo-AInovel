...
export class ContextProxy {
	...
	private getAllSecretState(): SecretState {
		const secretState: SecretState = {
			codeIndexOpenAiKey: this.getSecret("codeIndexOpenAiKey"),
			codeIndexQdrantApiKey: this.getSecret("codeIndexQdrantApiKey"),
		}

		// 添加其他键
		SECRET_STATE_KEYS.forEach((key) => {
			if (key !== "codeIndexOpenAiKey" && key !== "codeIndexQdrantApiKey") {
				secretState[key] = this.getSecret(key) as string | undefined
			}
		})
