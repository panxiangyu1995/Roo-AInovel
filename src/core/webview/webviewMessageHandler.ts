...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "autoUpdateEnabled":
			await updateGlobalState("autoUpdateEnabled", message.bool ?? false)
			await provider.postStateToWebview()
			break
		case "enhancePrompt":
			if (message.text) {
		...
		case "refreshTitleBar":
			...
			break
		case "getFrameworkSettings":
			// 从配置中获取框架设置
			const frameworkSettings = {
				autoUpdateEnabled: getGlobalState("autoUpdateEnabled") || false
			}
			provider.postMessageToWebview({ 
				type: "frameworkSettings", 
				values: frameworkSettings 
			})
			break
		case "updateFrameworkSettings":
			// 更新框架设置
			if (message.settings) {
				if (typeof message.settings.autoUpdateEnabled === 'boolean') {
					await updateGlobalState("autoUpdateEnabled", message.settings.autoUpdateEnabled)
				}
			}
			break
		case "activateNovelFramework":
			// 激活小说框架文件
			if (message.path) {
				try {
					const frameworkPath = message.path
					const rootPath = provider.cwd || process.cwd()
					...
					if (await fileExistsAtPath(fullPath)) {
						// 读取框架文件内容
						const frameworkContent = await fs.readFile(fullPath, "utf-8")
						
						// 获取文件名和框架名
						const fileName = path.basename(frameworkPath)
						const frameworkName = fileName.replace(/\.\w+$/, '') // 移除扩展名
						
						// 创建以.rules.开头的标准框架文件名
						const rulesFileName = `.rules.${frameworkName}framework.md`
						const rulesFilePath = path.join(rootPath, rulesFileName)
						
						// 先删除可能存在的旧的框架文件
						try {
							// 查找所有.rules.开头且包含"框架"或"framework"的文件
							const dirEntries = await fs.readdir(rootPath, { withFileTypes: true })
							const oldFrameworkFiles = dirEntries
							...
							// 删除旧的框架文件
							for (const oldFile of oldFrameworkFiles) {
								if (oldFile !== rulesFileName) { // 不删除即将创建的文件
									const oldFilePath = path.join(rootPath, oldFile)
									await fs.unlink(oldFilePath)
									provider.log(`删除旧的框架文件: ${oldFile}`)
								}
							}
						} catch (error) {
							provider.log(`检查旧框架文件时出错: ${error.message}`)
							// 继续处理，即使删除旧文件失败
						}
						
						// 保存新的框架文件
						await fs.writeFile(rulesFilePath, frameworkContent, "utf-8")
						
						// 通知用户
						provider.postMessageToWebview({ 
							type: "action", 
							text: `已成功将"${fileName}"设置为小说框架。框架将对除小说框架模式外的所有模式生效。` 
						})
					} else {
						provider.postMessageToWebview({ 
							type: "action", 
							text: `错误：找不到文件"${frameworkPath}"。` 
						})
					}
				} catch (error) {
					provider.postMessageToWebview({ 
						type: "action", 
						text: `设置框架时出错：${error.message}` 
					})
				}
			}
			break
