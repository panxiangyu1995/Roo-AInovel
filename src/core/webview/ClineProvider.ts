	...
	implements vscode.WebviewViewProvider, TelemetryPropertiesProvider
{
	...
	public static readonly sideBarId = `roo-ainovel.SidebarProvider`
	public static readonly tabPanelId = `roo-ainovel.TabPanelProvider`
	...
	*/
	...
	async resolveWebviewView(webviewView: vscode.WebviewView | vscode.WebviewPanel) {
		...
		if (inTabMode) {
			...
			// 延迟注册标题栏按钮，确保webview已完全加载
			setTimeout(async () => {
				this.log("Refreshing title bar buttons for tab view")
				// 尝试刷新标题栏按钮
				try {
					await vscode.commands.executeCommand("workbench.action.focusActiveEditorGroup");
					await vscode.commands.executeCommand("workbench.action.refreshEditorTitle");
				} catch (err) {
					this.log(`Error refreshing title bar: ${err}`);
				}
			}, 1000); // 延迟1秒
	...
	async postStateToWebview() {
		const webviewState = await this.getStateToPostToWebview()

		// 确保codebaseIndexConfig字段使用正确的类型
		if (webviewState.codebaseIndexConfig) {
			// 不再需要转换字段名，直接使用原始字段名
			const codebaseConfig = webviewState.codebaseIndexConfig as any

			// 使用原始字段名
			webviewState.codebaseIndexConfig = {
				codebaseIndexEnabled: codebaseConfig.codebaseIndexEnabled,
				codebaseIndexQdrantUrl: codebaseConfig.codebaseIndexQdrantUrl,
				codebaseIndexEmbedderProvider: codebaseConfig.codebaseIndexEmbedderProvider,
				codebaseIndexEmbedderBaseUrl: codebaseConfig.codebaseIndexEmbedderBaseUrl,
				codebaseIndexEmbedderModelId: codebaseConfig.codebaseIndexEmbedderModelId,
			}
		}

		this.postMessageToWebview({ type: "state", state: webviewState as any })
	}
