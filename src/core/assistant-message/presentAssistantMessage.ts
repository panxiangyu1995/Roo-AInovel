...
import { novelCommentTool } from "../tools/novelCommentTool"
import { formatConverterTool } from "../tools/formatConverterTool"
import { novelFrameworkRefineTool } from "../tools/novelFrameworkRefineTool"
import { contentExpansionTool } from "../tools/contentExpansionTool"
		...
		case "tool_use":
			const toolDescription = (): string => {
				switch (block.name) {
					...
					case "novel_comment":
						return `[${block.name} for '${block.params.path}' at line ${block.params.line}]`
					case "new_task": {
					...
					case "inspiration":
						return `[${block.name} for '${block.params.path}']`
					case "imitation":
						return `[${block.name} for '${block.params.path || "provided text"}']`
					case "novel_to_script":
						return `[${block.name} for '${block.params.path}' type: ${block.params.text?.includes("type=") ? block.params.text.match(/type=([^\s&]+)/)?.[1] : block.params.text || "movie"}]`
					case "novel_analysis":
						return `[${block.name} for '${block.params.path}' type: ${block.params.type || "full"}]`
					case "novel_content_search":
						return `[${block.name} for '${block.params.path}' query: '${block.params.query}']`
					default:
						return `[${block.name}]`
				}
			}
			...
			switch (block.name) {
				...
				case "novel_comment":
					await novelCommentTool(
						cline,
						block as any,
						(message: string) => askApproval("tool", message),
						(error: Error) => handleError("adding comment", error),
						pushToolResult,
						() => removeClosingTag("content", ""),
					)
					break
				case "format_converter":
					await formatConverterTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag)
					break
				case "list_code_definition_names":
				...
				case "attempt_completion":
					...
					break
				case "novel_framework_refine":
					await novelFrameworkRefineTool(
						cline,
						block as any,
						(message: string) => askApproval("followup", message),
						handleError,
						pushToolResult,
						() => ""
					)
				...
				case "inspiration":
					...
					break
				case "imitation":
					await imitationTool(cline, block as any, askApproval, handleError, pushToolResult, removeClosingTag)
					break
				case "novel_to_script":
					await novelToScriptTool(
						cline,
						block as any,
						askApproval,
						handleError,
				...
				case "novel_analysis":
					// 使用动态导入
					const novelAnalysisTool = require("../tools/novelAnalysisTool").novelAnalysisTool
					await novelAnalysisTool(
						cline,
						block as any,
						askApproval,
						handleError,
				...
				case "comic_generator":
					const { comicGeneratorTool } = require("../tools/comicGeneratorTool")
					await comicGeneratorTool(
						cline,
						block as any,
						askApproval,
						handleError,
